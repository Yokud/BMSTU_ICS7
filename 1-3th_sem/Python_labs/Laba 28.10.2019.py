...
def Weddle(a, b, n):
    ...
    if n % 6 == 0:
        while count < limit:
            I += 0.3*h*(F(ll)+F(ll+2*h)+5*F(ll+h)+6*F(ll+3*h)+F(ll+4*h)+\
                5*F(ll+5*h)+F(ll+6*h))
            ll += 6*h
            count += 1
    else:
...
s1 = '{:^13.7g}║'
if type(Weddle(a, b, n1)) == str:
    s1 = '{:^13s}║'
print(s1.format(Weddle(a, b, n1)), end = '')
s1 = '{:^13.7g}║'
if type(Weddle(a, b, n2)) == str:
    s1 = '{:^13s}║'
print(s1.format(Weddle(a, b, n2)))
print('╟', '─'*14, '╫', '─'*13,'╫', '─'*13,  '╢', sep = '')
print('║{:^14s}║{:^13.7g}║{:^13.7g}║'.format('Метод Л. Прям.',\
                                             Left_Rectangles(a, b, n1),\
                                             Left_Rectangles(a, b, n2)))
print('╙', '─'*14, '╨', '─'*13, '╨', '─'*13, '╜', sep = '')


# Вычисление интеграла функции с точностью eps и погрешностей
if n1 % 6 == 0 and n2 % 6 == 0:
    if abs(Left_Rectangles(a, b, n1) - Left_Rectangles(a, b, n2)) >\
       abs(Weddle(a, b, n1) - Weddle(a, b, n2)):
        n = min(n1, n2)
        while abs(Left_Rectangles(a, b, n) - Left_Rectangles(a, b, 2*n)) > eps:
            I = Left_Rectangles(a, b, n)
            n *= 2
        print('Площадь под графиком по методу левых прямоугольников с'+\
              ' точностью eps: {:.7g}'.format(I))
        abs_p = I - Left_Rectangles(a, b, min(n1, n2))
        otn_p = abs_p/I
        print('Абсолютная погрешность: {:.7g}'.format(abs_p))
        print('Относительная погрешность: {:.7g}'.format(otn_p))
    else:
        n = min(n1, n2)
        while abs(Weddle(a, b, n) - Weddle(a, b, 2*n)) > eps:
            I = Weddle(a, b, n)
            n *= 2
        print('Площадь под графиком по методу Уэддла с точностью eps: {:.7g}'\
              .format(I))
        abs_p = I - Weddle(a, b, min(n1, n2))
        otn_p = abs_p/I
        print('Абсолютная погрешность: {:.7g}'.format(abs_p))
        print('Относительная погрешность: {:.7g}'.format(otn_p))
else:
