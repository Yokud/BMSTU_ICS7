...
namespace lab_09
{
    class Cutter
    {
        ...
        {
            if (W.Count == 0)
                throw new Exception("Отсекатель не введён");
        ...
        public List<PointF> CutSutherlandHodgman(List<PointF> P)
        {
            ...
            for (int i = 0; i < W.Count; i++)
            {
                ...
                {
                    Vector D = new Vector(P[j], P[j-1]);
                    Dsk = Vector.ScalarMultiplication(D, normal[i]);

                    if (Dsk != 0) // Провека, что не точка и не параллельно
                    {
                        Vector WV = new Vector(P[j-1], W[i]);
                        Wsk = Vector.ScalarMultiplication(WV, normal[i]);
                        t = -Wsk / Dsk;
                        if (t >= 0 && t <= 1)
                        {
                            I = new Segment(P[j-1], P[j]).GetDot(t);
                            Q.Add(I);
                        }
                    }
