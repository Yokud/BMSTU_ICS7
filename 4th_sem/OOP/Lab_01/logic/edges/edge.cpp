...
{
    error_t err = OK;
    if (fscanf(f, "%zu", &edge.v1) != 1)
        err = READ_EDGES_ERROR;

    if (err == OK)
        edge.v1--;
    if (err == OK && (fscanf(f, "%zu", &edge.v2 ) != 1))
        err = READ_EDGES_ERROR;
    if (err == OK)
        edge.v2--;
...
{
    error_t err = OK;
    if (edge.v1 > max_vertex || edge.v1 < 0)
        err = INCORRECT_MODEL;
    if (err == OK && (edge.v2 > max_vertex || edge.v2 < 0))
        err = INCORRECT_MODEL;
    return err;
}

void write_edge(FILE * const f, const edge_t &edge)
{
    fprintf(f, "%zu %zu", edge.v1 + 1, edge.v2 + 1);
}
