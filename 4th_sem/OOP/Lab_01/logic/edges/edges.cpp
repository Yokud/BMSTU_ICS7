...
static error_t read_edges_num(size_t &n, FILE *const f)
{
    error_t err = OK;
...
static void write_edges_num(FILE * const f, size_t n)
{
    fprintf(f, "%zu\n", n);
}
...
{
    write_edges_num(f, edges.n);
    for (size_t i = 0; i < edges.n; i++)
    {
        write_edge(f, edges.edges_arr[i]);
        fprintf(f, "\n");
    }
}
...
error_t are_correct_edges(const edges_t &edges, const points3D_t &points)
{
    error_t err = OK;
    for (size_t i = 0; err == OK && i < edges.n; i++)
        err = is_correct_edge(edges.edges_arr[i], points.points_count);
    return err;
}
