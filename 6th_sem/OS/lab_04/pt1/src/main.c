...
void printCMDLINE()
{
    ...
    fprintf(f, "\nCMDLINE (полная командная строка для процесса)\n");
    fprintf(f, "%s\n", buf);
...
void printCOMM()
{
    ...
    int len = fread(buf, 1, BUFFSIZE, file);
    buf[len - 1] = 0;
    fprintf(f, "\nCOMM (значение comm процесса (имя команды процесса))\n%s\n", buf);
...
void printEXE()
{
    ...
    fprintf(f, "\nEXE (фактический путь к выполняемой программе) \n");
...
void printROOT()
{
    ...
    fprintf(f, "\nROOT (корневой каталог процесса):\n");
...
void printIO()
{
    ...
    int len = fread(buf, 1, BUFFSIZE, file);
    buf[len - 1] = 0;
    fprintf(f, "\nIO (статистика ввода/вывода процесса):\n%s\n", buf);
...
void printSTATM()
{
    char *statmMeanings[] = {"size", "общее число страниц выделенное процессу в виртуальной памяти",
                             ...
                             "shared", "количество общих резидентных страниц",
