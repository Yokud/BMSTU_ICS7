...
namespace lab_5_Conveyor
{
    static class Analysis
    {
        ...
        public static void AnalyseDeltaFunc(int nIter, int nLines, int nData, int minDelay, int maxDelay, int step, Action<List<Args>, int, int, int, Action<int, Args, int, bool>> func)
        {
            Stopwatch stopWatch = new Stopwatch();
            ...
            for (int currDelay = minDelay; currDelay <= maxDelay; currDelay += step)
            {
                ...
                {
                    stopWatch.Start();
                    func(allData, nLines, minDelay, maxDelay, Program.Act);
                    stopWatch.Stop();

                    ts += stopWatch.Elapsed.Ticks;
                }
                Console.WriteLine(ts / nIter);
            }
